...
{
protected:
  rclcpp::Publisher<diagnostic_msgs::msg::DiagnosticArray>::SharedPtr diagnostic_pub_;   //!< Diagnostic publisher.
  diagnostic_msgs::msg::DiagnosticArray diagnostic_msg_;   //!< Diagnostic message to publish.
...
public:
   ...
   */
  DiagnosticHelper(std::shared_ptr<rclcpp::Node> node, const std::string& name, const std::string& hardware_id);
   ...
   */
  void reportData(const rclcpp::Time stamp = rclcpp::Time());  // When the checksum is ok
   ...
   */
  void reportValidData(const rclcpp::Time stamp = rclcpp::Time());  // When the data can be used
   ...
   */
  void publish(const rclcpp::Node::SharedPtr node,const rclcpp::Time stamp = rclcpp::Time());
};
/** @} */
}  // namespace ros
}  // namespace cola2
