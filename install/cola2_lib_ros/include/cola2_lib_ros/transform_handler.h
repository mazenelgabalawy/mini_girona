...
private:
   ...
   */
  Eigen::Isometry3d tfTransformToEigen(const geometry_msgs::msg::TransformStamped& trans) const;

public:
  /**
   * @brief Constructor that queries the namespace to know vehicle frame.
   */
  TransformHandler(const rclcpp::Node::SharedPtr& node);
   ...
   */
  TransformHandler(const rclcpp::Node::SharedPtr& node,const std::string& frame);
  ...
  [[deprecated("use the version with Eigen::Isometry3d")]] bool getDynamicTransform(
      const std::string& frame, Eigen::Affine3d& transform, const rclcpp::Time& time = rclcpp::Time(0)) const;
  /**
   * @brief Get a transform by querying the listener.
   *
   * @param frame Frame.
  ...
  bool getDynamicTransform(const std::string& frame, Eigen::Isometry3d& transform,
                             const rclcpp::Time& time = rclcpp::Time(0)) const;
  /**
