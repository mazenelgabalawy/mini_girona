...
protected:
  ...
  int navigator_type_ = 1;                       //!< 1 --> EKFPositionVelocity / 2 --> EKFPosition / 3 --> EKFSurface2d
...
public:
   ...
   */
  explicit EKFBase(const unsigned int initial_state_vector_size, const unsigned int navigator_type);
...
protected:
   ...
   */
  void setPositionXY(const Eigen::Vector2d &xy);
  /**
   * \brief Compute prediction matrices fx_, A_, W_.
   *
   * \param dt Time increment since last prediction
   */
  void computePredictionMatrices(const double dt);
  /**
   ...
   */
  bool updatePositionXY(const double t, const Eigen::Vector2d &pose_xy, const Eigen::Matrix2d &cov);
  /**
   ...
   * \return Success of the update
   */
  bool updatePositionZ(const double t, const Eigen::Vector1d &pose_z, const Eigen::Matrix1d &cov);
  /**
   ...
   * \return Success of the update
   */
  bool updateOrientation(const double t, const Eigen::Vector3d &rpy, const Eigen::Matrix3d &cov);
  /**
   ...
   * \return Success of the update
   */
  bool updateOrientationRate(const double t, const Eigen::Vector3d &rate, const Eigen::Matrix3d &cov);
