#ifndef COLA2_NAV_EKF_BASE_ROS2_H_
#define COLA2_NAV_EKF_BASE_ROS2_H_
...
/**
 * \brief EKF including all the ROS2 functionalities.
 * It loads parameters, publishes state
 */
class EKFBaseROS2 : public EKFBase
{
private:
    bool prevUpdatePositionXY(const double t, const Eigen::Vector2d &pose_xy, const Eigen::Matrix2d &cov);
    bool prevUpdatePositionZ(const double t, const Eigen::Vector1d &pose_z, const Eigen::Matrix1d &cov);
    bool prevUpdateOrientation(const double t, const Eigen::Vector3d &rpy, const Eigen::Matrix3d &cov);
    bool prevUpdateVelocity(const double t, const Eigen::Vector3d &vel, const Eigen::Matrix3d &cov,
                        const bool from_dvl = true);
    bool prevUpdateOrientationRate(const double t, const Eigen::Vector3d &rate, const Eigen::Matrix3d &cov);

protected:
    // ROS2 transforms and frames
    ...
    rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr srv_reset_navigation_;        //!< realoads params and sets the filter to initial state
    ...
    rclcpp::Client<std_srvs::srv::Trigger>::SharedPtr safety_reset_client_;
    ...
    rclcpp::Node::SharedPtr node_; //!< ROS2 node what was the node handler in ROS
...
public:
    ...
    /**
     * \brief Load config from ROS2 param server.
     *
     * \param show Print on screen the loaded parameters
     */
    std_srvs::srv::Trigger::Response getConfig(const bool show = false);
    void initBase(const rclcpp::Node::SharedPtr &node);
     ...
     */
    explicit EKFBaseROS2(const unsigned int initial_state_vector_size, const unsigned int navigator_type, const bool online = true);
    /**
     * \brief Destructor.
     */
    virtual ~EKFBaseROS2() noexcept;

    // *****************************************
    // Updates from ROS2 messages
    // *****************************************
     ...
     */
    void updatePositionGPSMsg(const sensor_msgs::msg::NavSatFix::SharedPtr msg);
    bool updatePositionGPSMsgImpl(const sensor_msgs::msg::NavSatFix::SharedPtr msg);
     ...
     * \param msg Recevied message
     */
    void updatePositionUSBLMsg(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg);
    bool updatePositionUSBLMsgImpl(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg);
    /**
     * \brief Callback to update the filter with a depth message
     *
     * \param msg Recevied message
     */
    void updatePositionDepthMsg(const sensor_msgs::msg::FluidPressure::SharedPtr msg);
    /**
     * \brief Callback to update the filter with a DVL message
     *
     * \param msg Recevied message
     */
    void updateVelocityDVLMsg(const cola2_msgs::msg::DVL::SharedPtr msg);
    /**
     * \brief Callback to update the filter with a fallback DVL message
     *
     * This callback only works when invalid DVL messages have been received at least for config_.dvl_fallback_delay_
     *
     * \param msg Recevied message
     */
    void updateVelocityDVLFallbackMsg(const cola2_msgs::msg::DVL::SharedPtr msg);
    /**
     ...
     */
    void updateVelocityDVLMsgImpl(const cola2_msgs::msg::DVL::SharedPtr msg, const bool from_dvl);
    /**
     * \brief Callback to update the filter with an IMU message
     *
     * \param msg Recevied message
     */
    void updateIMUMsg(const sensor_msgs::msg::Imu::SharedPtr msg);
    // Others
    /**
     ...
     * \param msg Recevied message
     */
    void updateSoundVelocityMsg(const cola2_msgs::msg::Float32Stamped::SharedPtr msg);
    /**
     * \brief Callback to update the filter with an altitude message
     *
     * \param msg Recevied message
     */
    void updateAltitudeMsg(const sensor_msgs::msg::Range::SharedPtr msg);
     ...
     */
    bool srvReloadNED(const std::shared_ptr<std_srvs::srv::Trigger::Request> req,
     ...
     * \return Correct processing of the service
     */
    bool srvResetNavigation(const std::shared_ptr<std_srvs::srv::Trigger::Request> req,
                            std::shared_ptr<std_srvs::srv::Trigger::Response> res);
     ...
     * \return Correct processing of the service
     */
    bool srvSetDepthSensorOffset(const std::shared_ptr<std_srvs::srv::Trigger::Request> req,
                                 std::shared_ptr<std_srvs::srv::Trigger::Response> res);
     ...
     *
     * The format of each transform is a line in the file with the following parts:
...
#endif // COLA2_NAV_EKF_BASE_ROS2_H_
